/**
 * GitHub OAuth proxy for Decap CMS, running as a Cloudflare Worker.
 *
 * Vendored (unmodified) from https://github.com/sveltia/sveltia-cms-auth —
 * see that repo for license terms. Deployed here because relying on
 * Netlify's implicit OAuth proxy (api.netlify.com/auth) for a site that
 * isn't actually hosted on Netlify is an unofficial community hack with a
 * known-flaky postMessage handshake back to a foreign origin; this is
 * Decap's own documented alternative — a small dedicated OAuth proxy.
 *
